...
async fn test_wal_restarts_never_overites_and_compactor_reads_all_batches() {
    ...
    // REBOOT SIMULATION: Boot WAL again and write batch 2 (3 items)
    {
        let (wal, handle) = WalEngine::start(test_wal_path.to_string()).await;
        for i in 3..6 {
            wal.append(create_mock_log(i, "Batch 2")).await;
        }
    ...
    assert!(
        size_after_run2 > size_after_run1, "CRIT-02 REGRESSION: WAL did not append on reboot! File was overritten!"
    );  
    ...
    while offset + 8 <= buffer.len() {
        ...
        assert_eq!(actual_crc, expected_crc, "CRIT-01 REGRESSION: CRC mismatch on batch {} at offset {}!", total_baches_parsed + 1, offset);
    ...
    assert_eq!(total_baches_parsed, 2, "Compactor should have parsed exactly 2 distict batches");
