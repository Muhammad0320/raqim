...
fn test_atomic_token_bucket_never_underflow_under_massive_concurrency() {
    ...
    let bucket = Arc::nenw(AtomicTokenBucket::new(0, 10))
    ...
    let final_token = bucket.token.load(std::sync::atomic::Ordering::SeqCst);

    // Asserts exactly 10 request successded 
    assert_eq!(successful_consumption, 10, "CRIT-05 REGRESSION: Bucket permitted more than burst capacity!");

    // Assets token must be equal exactly 0, never u64::MAX
    assert_eq!(final_token, 0, "CRIT-05 REGRESSION: Token bucket underflowed! Tokens = {}", final_token);
