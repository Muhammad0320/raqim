...
export default function BillingPage() {
  ...
  const handleUpgrade = () => {
    ...
    setTimeout(() => {
      ...
      setToastMessage("Redirecting to Stripe... Payment successful. Swarm infrastructure upgraded.");
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-200 font-sans selection:bg-zinc-800 selection:text-white p-8">
      ...
      <div className="max-w-5xl mx-auto space-y-16 py-8">
        ...
        {/* 2. The Upgrade Flow (Stripe Integration UI) */}
        ...
        {showVendingMachine && (
          <>
            {/* 3. The Sovereign License Vending Machine */}
            <section className="space-y-6 pt-8 border-t border-zinc-800/50">
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-lg font-medium text-white mb-1">Sovereign License Key</h2>
                  <p className="text-sm text-zinc-500 max-w-xl">Configure your daemon by copying this key to your local configuration. Do not share this key.</p>
                </div>
              </div>
              ...
              <div className="border border-zinc-800 rounded-xl bg-zinc-950 p-6 relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center z-10 gap-4 mb-6">
                  <div className="flex items-center space-x-2 text-xs font-mono text-zinc-400 bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800">
                    <InfoIcon />
                    <span>Your license automatically rotates every 7 days via Telemetry Ping.</span>
                  </div>
                  
                  <div className="flex flex-wrap gap-2">
                    <button 
                      onClick={() => setShowKey(!showKey)}
                      className="flex items-center space-x-2 text-sm px-4 py-2 rounded-lg border border-zinc-800 hover:border-zinc-700 bg-zinc-900 hover:bg-zinc-800 transition-all text-zinc-300"
                    >
                    ...
                    <button 
                      onClick={handleCopy}
                      className="flex items-center space-x-2 text-sm px-4 py-2 rounded-lg bg-white text-zinc-950 hover:bg-zinc-200 transition-all font-medium"
                    >
                      <CopyIcon />
                      <span>{copied ? "Copied" : "Copy to raqim.toml"}</span>
                    </button>
                  </div>
                </div>
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Module 1 */}
                <div 
                  className={`border rounded-xl p-5 cursor-pointer transition-all duration-300 flex flex-col justify-between min-h-[140px] group ${modules.aegis ? 'border-zinc-500 bg-zinc-900 shadow-[0_0_15px_rgba(255,255,255,0.03)]' : 'border-zinc-800 bg-zinc-950/50 hover:border-zinc-700'}`}
                ...
                {/* Module 4 */}
                <div 
                  className={`border rounded-xl p-5 cursor-pointer transition-all duration-300 flex flex-col justify-between min-h-[140px] group ${modules.temporal ? 'border-zinc-500 bg-zinc-900 shadow-[0_0_15px_rgba(255,255,255,0.03)]' : 'border-zinc-800 bg-zinc-950/50 hover:border-zinc-700'}`}
