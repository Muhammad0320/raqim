        ...
        match method {
            ...
            "tools/call" => {
                ...
                if name == "commit_thought" {
                    ...
                    let agent_id_hex = args.get("agent_id_hex").and_then(|v| v.as_str()).ok_or_else(|| mcp_rust_sdk::Error::Other("Missing agent_id_hex".to_string()) )?; 
                    let agent_id_bytes = hex::decode(agent_id_hex).map_err(|_| mcp_rust_sdk::Error::Other("Invalid Hex".to_string()) )?;
                    ...
                    let url = format!("{}/v1/swarm/memory?namespace={}&query={}", self.daemon_http_url, intent_path, query);

                    // AXUM HTTP RAG CALL. 
                    let response = self.http_client.get(&url).header("Authorization", format!("Bearer {}", self.license_key)).send().await.map_err(|e| mcp_rust_sdk::Error::Other(e.to_string()) )?;

                    if response.status().is_success() {
                        let memories: Vec<String> = response.json().await.unwrap_or_default();

                        return Ok(json!({ "content": [ {"type": "text", "text": format!("Retreived: \n{}", memories.join("\n"))}]}));
                    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let key_path = std::env::var("RQM_MCP_KEY_PATH").unwrap_or_else(|_| "./keys/mcp_private.pem".to_string());
    let daemon_url = std::env::var("RQM_DEAMON_URL").unwrap_or_else(|_| "http://127.0.0.1:8081".to_string() );

    let (transport, _message_sender) = StdioTransport::new();

    let handler = Arc::new(RaqimHandler::new(&key_path, &daemon_url));
